...
const LiveTracking = () => {
  ...
  const LIVE_FLEET = [
    {
      ...
      speed: 68,
      lastPing: "2m ago",
      location: "NH-44, 42km before Nagpur",
      lat: 21.14,
      lng: 79.08,
      driver: "Mani Kumar",
      trip: "TRP-2025-0041",
    ...
    },
    {
      ...
      speed: 0,
      lastPing: "5m ago",
      location: "HPCL, Krishnagiri, TN-38",
      lat: 12.51,
      lng: 78.21,
      driver: "Selvam R",
      trip: "TRP-2025-0042",
    ...
    },
    {
      ...
      speed: 55,
      lastPing: "1m ago",
      location: "NH-7, 180km from Delhi",
      lat: 28.67,
      lng: 77.22,
      driver: "Vinoth S",
      trip: "TRP-2025-0045",
    ...
    },
    {
      ...
      speed: 0,
      lastPing: "3h ago",
      location: "Ganesh Auto Works, Nagpur",
      lat: 21.14,
      lng: 79.09,
      driver: "Ramesh P",
      trip: "—",
  ...
  const fleetStatusData = [
    ...
    {
      label: "Maintenance",
      value: LIVE_FLEET.filter((item) => item.status === "In Maintenance")
        .length,
      color: "#8B5CF6",
    },
    {
    ...
    <div>
      <div className="tracking-container">
        <div>
          ...
          <p className="tracking-sub-title">
            Real-time fleet visibility — every vehicle, every moment
          </p>
        </div>
        <div className="tracking-buttons">
          <button className="control-btn tracking-btn-b">
            <Ic n="rotate" s={13} c="var(--blue)" /> Refresh All
          </button>
        </div>
      </div>
      ...
      <div className="row g-3">
        ...
        <div className="col-lg-8">
          <div className="tracking-panel-container">
            <div className="tracking-panel-placeholder">
              <img src={Map} alt="Fleet Map" className="tracking-map-image" />
              ...
              {LIVE_FLEET.map((v, i) => (
                ...
                >
                  <div
                    className="tracking-panel-avatar"
                    style={{ background: statusColors2[v.status] }}
                  />

                  <div
                    className="tracking-panel-dot-map"
                    style={{ background: statusColors2[v.status] }}
                  >
                    {v.id.split(" ")[0]}
                  </div>
        ...
        <div className="col-lg-4">
          ...
          >
            <div className="tracking-fleet-title">
              Fleet ({LIVE_FLEET.length} vehicles)
            </div>
            {LIVE_FLEET.map((v) => (
              <div
                key={v.id}
                className="tracking-fleet-box"
                onClick={() => setSelVehicle(v)}
                style={{
                  background:
                    selVehicle?.id === v.id
              ...
              >
                <div className="tracking-fleet-row">
                  <span
                    className="mono tracking-fleet-id"
                    style={{
                      color:
                        selVehicle?.id === v.id
                          ? "var(--accent)"
                  ...
                  <div className="tracking-fleet-col">
                    <div
                      className="tracking-dot tracking-pulse"
                      style={{ background: statusColors2[v.status] }}
                    />
                    <span
                      className="tracking-fleet-status"
                      style={{ color: statusColors2[v.status] }}
                    >
                      {v.status}
                    </span>
                  </div>
                ...
                {v.speed > 0 && (
                  <div className="tracking-fleet-speed">
                    🏎️ {v.speed} km/h · {v.lastPing}
                  </div>
                )}
              </div>
            ))}
          ...
          {selVehicle ? (
              <div className="control-card-box">
                <div className="tracking-card-header">
                  <div>
                    <div className="mono tracking-vehicle-id">
                      {selVehicle.id}
                    </div>
                    <div className="tracking-vehicle-driver">
                      {selVehicle.driver} ·{" "}
                      {selVehicle.trip !== "—"
                        ? selVehicle.trip
                        : "Not on trip"}
                  ...
                  <div className="tracking-vehicle-box">
                    <div
                      className="tracking-dot tracking-pulse"
                      style={{ background: statusColors2[selVehicle.status] }}
                    />
                    <span
                      className="tracking-vehicle-status"
                      style={{ color: statusColors2[selVehicle.status] }}
                    >
                      {selVehicle.status}
                    </span>
                  </div>
                ...
                <div className="row g-3" style={{ marginBottom: "12px" }}>
                  {[
                    ...
                    {
                      l: "KM Covered",
                      v: `${selVehicle.kmCovered}/${selVehicle.kmTotal} km`,
                    },
                ...
                <div className="tracking-location-details">
                  <span className="tracking-selVehicle-location">
                    Location:{" "}
                  </span>
                  {selVehicle.location}
                </div>
                {selVehicle.route !== "—" && (
                  <div style={{ marginTop: "8px" }}>
                    <div className="tracking-route-header">
                      <span className="tracking-route-title">
                        Route progress
                      </span>
                      <span className="tracking-route-distance">
                        {selVehicle.kmCovered} / {selVehicle.kmTotal} km
                      </span>
                    </div>
                    <div className="control-pbar" style={{ height: "6px" }}>
                      <div
                        className="control-pfill tracking-pfill-pb"
                        style={{
                          width: `${(selVehicle.kmCovered / selVehicle.kmTotal) * 100}%`,
                        }}
                      />
                    </div>
                  </div>
              ...
              <div className="control-card-box tracking-truck-details">
                ...
                <div className="tracking-truck-sub-title">
                  Click a vehicle to see live details
                </div>
              </div>
            )}
      </div>
