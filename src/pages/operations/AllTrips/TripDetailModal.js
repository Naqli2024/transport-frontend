...
const TripDetailModal = ({
  ...
  const margin =
    t.freight > 0 ? ((profit / t.freight) * 100).toFixed(1) : "0.0";
    ...
    <div>
      ...
      >
        <DialogContent sx={{ p: 0 }}>
          <div
            className="trip-modal-container"
            style={{
              background: `linear-gradient(135deg,${isOwn ? "#052E16,#064E3B" : "#1E1B4B,#2E1065"})`,
            }}
          >
            <div className="trip-modal-main">
              <div>
                <div className="trip-modal-header">
                  ...
                  <span className="control-badge trip-modal-badge-id">
                    {t.id}
                  </span>
                </div>
                ...
                <div className="trip-modal-badges">
                  {jt && (
                    <span
                      ...
                      style={{ background: jt.color + "33", color: jt.color }}
                    >
                      {jt.icon} {jt.label}
                    </span>
                  )}
                  <span
                    ...
                    style={{
                      background: isOwn
                        ? "var(--green)" + "33"
                        : "var(--purple)" + "33",
                      color: isOwn ? "var(--green)" : "var(--purple)",
                    }}
                  >
                    {isOwn ? "Own Fleet" : "Vendor Trip"}
                  </span>
                  <span className="control-badge trip-modal-badge-load">
                    {t.loadType}
                  </span>
                  <span className="control-badge trip-modal-badge-load">
                    {t.weight}
                  ...
                  <span
                    ...
                    style={{
                      background:
                        (STATUS_COLORS[t.status] || "var(--textSub)") + "44",
              ...
              <div className="trip-modal-right">
                ...
                <button
                  className="control-btn trip-modal-icon-btn"
                  onClick={onClose}
                >
                  <Ic n="x" s={14} c="#fff" />
                </button>
                ...
                return (
                  <div key={s} className="trip-modal-timeline-sheet">
                    <div className="trip-modal-timeline-sheet-main">
                      <div
                        className="trip-modal-timeline-avatar"
                        style={{
                          background: curr
                            ? "var(--accent)"
                            : done
                      ...
                      <div
                        className="trip-modal-timeline-data"
                        style={{ color: col, fontWeight: curr ? 700 : 400 }}
                      >
                        {s}
                      </div>
                    </div>
                    {i < statusSeq.length - 1 && (
                      <div
                        className="trip-modal-timeline-seq"
                        style={{
          ...
          <div className="trip-modal-main-content">
            <div className="trip-modal-summary-grid">
              {[
                ...
                {
                  l: isOwn ? "Total Expenses" : "Vendor + Extras",
                  v: fmt(totalExp),
                  c: "var(--red)",
                  border: "var(--red)",
                },
                {
                  ...
                  border: profit >= 0 ? "var(--green)" : "var(--red)",
                },
                {
                  l: "Margin %",
                  v: `${margin}%`,
                ...
                {
                  l: isOwn ? "Advance Given" : "Vendor Cost",
                  v: isOwn ? fmt(t.advance) : fmt(t.vendorCost || 0),
                  c: "var(--orange)",
                  border: "var(--orange)",
                },
              ].map((k) => (
                <div
                  key={k.l}
                  className="trip-modal-summary-main"
                  style={{
                    border: `1px solid ${k.border}22`,
                    borderTop: `3px solid ${k.border}`,
                  }}
                >
                  <div className="trip-modal-summary-l">{k.l}</div>
                  <div
                    className="rj trip-modal-summary-v"
                    style={{ color: k.c }}
                  >
                    {k.v}
                  </div>
            ...
            <div className="row g-3" style={{ marginBottom: "16px" }}>
              <div className="col-lg-4">
                <div
                  className="trip-modal-trip-card"
                  style={{ height: "410px" }}
                >
                          ...
                          color:
                            k === "Trip ID"
                              ...
                              : k === "Customer"
                                ...
                                : "var(--text)",
                          fontFamily:
                            k === "Trip ID" ? "JetBrains Mono" : "inherit",
                          fontSize: k === "Trip ID" ? 11 : 12,
                        }}
                      >
                        {v}
                      </span>
              ...
              <div className="col-lg-4">
                ...
                >
                  <div className="section-title">
                    {isOwn ? "🚛 Vehicle & Driver" : "🤝 Vendor Details"}
                  </div>
                  {(isOwn
                    ? [
                        ...
                        [
                          "Diesel (litres)",
                          `${Math.round((t.expenses?.diesel || 0) / 100)} L`,
                        ],
                    ...
                    : [
                        ...
                        [
                          "Vendor Rate/km",
                          t.vendorRatePerKm ? `₹${t.vendorRatePerKm}/km` : "—",
                        ],
                    ...
                    <div key={k} className="trip-modal-trip-row">
                      ...
                      <span
                        className="trip-modal-trip-v"
                        style={{
                          fontFamily: ["Vehicle No.", "Driver ID"].includes(k)
                            ? "JetBrains Mono"
                            : "inherit",
                          fontSize: ["Vehicle No.", "Driver ID"].includes(k)
                            ? 11
                            : 12,
                          color:
                            k === "Vendor Name"
              ...
              <div className="col-lg-4">
                <div className="trip-modal-trip-card">
                  ...
                  {(isOwn
                    ...
                    <div key={k} className="trip-modal-ledger-row">
                      <span className="trip-modal-ledger-label">{k}</span>
                      <span className="trip-modal-ledger-dr">
                        {v > 0 ? fmt(v) : <span className="ledger-s">—</span>}
                      </span>
                    </div>
                  ))}
                  ...
                  <div className="trip-modal-profit-row">
                    ...
                    <span
                      className="trip-modal-profit-value rj"
                      style={{
                  ...
                  <div className="trip-modal-margin-row">
                    ...
                    <span
                      className="trip-modal-margin-value"
                      style={{
                        color:
                          parseFloat(margin) > 20
                            ? "var(--green)"
            ...
            {isOwn && (
              <div
                className="trip-modal-driver-settlement-card"
                style={{
                  background: driverOwes
                    ? "var(--redGlow)"
                    : "var(--greenGlow)",
                  border: `1px solid ${driverOwes ? "var(--red)" : "var(--green)"}33`,
                }}
              >
                <div className="trip-modal-driver-settlement-header">
                  <div>
                    <div className="trip-modal-driver-settlement-title">
                      👤 Driver Settlement — {t.driver}
                    </div>
                    <div className="trip-modal-driver-settlement-details">
                      {[
                        ...
                        [
                          "Total Expenses",
                          fmt(tripExpTotal(t.expenses)),
                          "#EF4444",
                        ],
                        [
                          driverOwes ? "Driver Returns" : "Pay Driver",
                          fmt(Math.abs(driverBalance)),
                          driverOwes ? "#EF4444" : "#10B981",
                        ],
                      ].map(([k, v, c]) => (
                        <div key={k}>
                          <span className="trip-modal-settlement-label">
                            {k}:{" "}
                          </span>
                          <span
                            className="trip-modal-settlement-value"
                            style={{ color: c }}
                          >
                            {v}
                          </span>
            ...
            <div className="row g-3" style={{ marginBottom: "16px" }}>
              {[
                ...
                <div className="col-lg-4" key={d.label}>
                  <div
                    className="trip-modal-doc-card"
                    style={{
                      border: `1px solid ${d.done ? d.color + "44" : "var(--border)"}`,
                    }}
                  >
                    <div
                      className="trip-modal-doc-icon-box"
                      style={{
                    ...
                    <div className="trip-modal-doc-content">
                      ...
                      <div
                        className="trip-modal-doc-value"
                        style={{ color: d.color }}
                      >
                        {d.value}
                      </div>
                    ...
                    {!d.done && (
                      <button
                        className="control-btn trip-modal-upload-btn"
                        style={{
            ...
            <div className="trip-modal-inspection-container">
              ...
              <div className="row g-3">
                {[
                  ...
                  {
                    type: "Post-Trip",
                    status: ["Post-Trip Done", "Invoiced", "Closed"].includes(
                      t.status,
                    )
                      ? "Completed"
                      : t.status === "Post-Trip Pending"
                        ...
                        : "Not Yet",
                    icon: "posttrip",
                    color: ["Post-Trip Done", "Invoiced", "Closed"].includes(
                      t.status,
                    )
                      ? "#10B981"
                      : t.status === "Post-Trip Pending"
                  ...
                  <div className="col-lg-6">
                    <div
                      className="trip-modal-inspection-card"
                      style={{ border: `1px solid ${ins.color}33` }}
                    >
                      <div
                        className="trip-modal-inspection-icon-box"
                        style={{
                      ...
                      <div className="trip-modal-inspection-content">
                        <div className="trip-modal-inspection-title">
                          {ins.type} Inspection
                        </div>
                        <div
                          className="trip-modal-inspection-status"
            ...
            >
              {t.status === "Pre-Trip Pending" && (
                <button
                  className="control-btn trip-modal-btn-green trip-modal-btn-trip"
                  onClick={() => {
                    onClose();
                    setShowInspect({ trip: t, type: "pre" });
                  }}
                >
                  <Ic n="pretrip" s={13} /> Pre-Trip Inspect
                </button>
              )}
              {t.status === "Post-Trip Pending" && (
                <button
                  className="control-btn trip-modal-btn-blue trip-modal-btn-trip"
                  onClick={() => {
                    onClose();
                    setShowInspect({ trip: t, type: "post" });
                  }}
                >
                  <Ic n="posttrip" s={13} /> Post-Trip Inspect
                </button>
              )}
              {["Started", "In Transit"].includes(t.status) && (
                <button
                  ...
                  onClick={() =>
                    setTrips((ts) =>
                      ts.map((x) =>
                        x.id === t.id ? { ...x, status: "Arrived" } : x,
                      ),
                    )
                  }
                >
                  <Ic n="check" s={13} /> Mark Arrived
                </button>
              )}
              {t.status === "Arrived" && (
                <button
                  ...
                  onClick={() =>
                    setTrips((ts) =>
                      ts.map((x) =>
                        x.id === t.id
                          ? { ...x, status: "Post-Trip Pending" }
                          : x,
                      ),
                    )
                  }
                >
                  <Ic n="check" s={13} /> Mark Delivered → Post-Trip
                </button>
              )}
              {t.status === "Post-Trip Done" && (
                <button
                  className="control-btn trip-modal-btn-cyan trip-modal-btn-trip"
                  onClick={() =>
                    setTrips((ts) =>
                      ts.map((x) =>
                        x.id === t.id ? { ...x, status: "Invoiced" } : x,
                      ),
                    )
                  }
                >
                  <Ic n="finance" s={13} /> Raise Invoice
                </button>
              )}
              <button className="control-btn trip-modal-btn-blue trip-modal-btn-trip">
                <Ic n="doc" s={13} /> Upload POD
              </button>
              {isOwn && (
                <button className="control-btn trip-modal-btn-orange trip-modal-btn-trip">
                  <Ic n="wallet" s={13} /> Settle Driver
                </button>
              )}
              <button
                className="control-btn trip-modal-btn-gh trip-modal-btn-trip"
