...
const Customers = () => {
  ...
  const { customers, summary, customerDetails, loading, error } = useSelector(
  ...
  const handleDelete = async () => {
    if (!selectedCustomer?._id || deleteLoading) return;
    ...
    try {
      ...
      if (response?.meta?.requestStatus === "fulfilled") {
        toast.success(
          response.payload?.message || "Customer deleted successfully",
        );
