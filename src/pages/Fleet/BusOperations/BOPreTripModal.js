...
const CATEGORIES = [
  { key:'safety_critical', label:'🚨 Safety Critical', colorCls:'red',    count:8  },
  { key:'electrical',      label:'⚡ Electrical',      colorCls:'blue',   count:7  },
  { key:'mechanical',      label:'🔧 Mechanical',      colorCls:'orange', count:6  },
  { key:'compliance',      label:'📋 Compliance Docs', colorCls:'green',  count:6  },
];
    ...
    <div className="bo-pt-overlay" onClick={onClose}>
      <div className="bo-pt-modal" onClick={e => e.stopPropagation()}>
        <div className="bo-pt-hdr">
          <div>
            <div className="bo-pt-title">🔍 Pre-Trip Safety Check — {bus.regNo}</div>
            <div className="bo-pt-subtitle">
              {totalChecks} checks across 4 categories · All Safety Critical items must pass to depart
            </div>
          </div>
        ...
        <div className="bo-pt-body">
          {criticalFailed && (
            <div className="bo-pt-danger-banner">
              ...
              <span>Bus <strong>CANNOT depart</strong> — resolve critical issues first</span>
            </div>
          )}
          <div className="bo-pt-progress-wrap">
            <div className="bo-pt-progress-meta">
              ...
              <span className="bo-pt-progress-pct">{totalPassed}/{totalChecks} passed</span>
            </div>
          ...
          {CATEGORIES.map(({ key, label, colorCls }) => {
            ...
            const passed = items.filter(c => ptData[`${bus.id}_${c.id}`] === 'ok').length;
            const issues = items.filter(c => ptData[`${bus.id}_${c.id}`] === 'issue').length;
            return (
              <div key={key} className="bo-pt-category">
                <div className="bo-pt-cat-head">
                  <div className={`bo-pt-cat-label bo-pt-cat-label--${colorCls}`}>{label}</div>
                  <div className="bo-pt-cat-counts">
                    {issues > 0 && <span className="bo-pt-count-issue">{issues} issue{issues > 1 ? 's' : ''}</span>}
                    <span className="bo-pt-count-passed">{passed}/{items.length} passed</span>
                  </div>
                </div>
                  ...
                  {items.map(item => {
                    ...
                    return (
                      ...
                      >
                        ...
                        {item.critical && <span className="bo-pt-crit-dot" title="Critical" />}
                      </button>
                    );
                  })}
                </div>
          ...
          {(totalPassed > 0 || totalIssues > 0) && (
            <div className="bo-pt-summary">
              ...
              <div className="bo-pt-sum-item bo-pt-sum--pending">
                <span className="bo-pt-sum-val">{totalChecks - totalPassed - totalIssues}</span>
        ...
        <div className="bo-pt-footer">
          ...
          <div className="bo-pt-footer-actions">
            <button className="bo-btn bo-btn-ghost" onClick={onClose}>Cancel</button>
            <button
